  ...
  <div>
    <div class="modal-content">
      <div class="modal-header bg-success">
        <h5 class="modal-title" id="exampleModalLongTitle">Guardar Venta</h5>
      </div>
      <div class="modal-body">
        <form @submit.prevent>
            ...
            <v-col>
              ...
              <div class="float-left">
                <v-btn
                  ...
                  color="success"
                  data-toggle="modal"
                  data-target="#modalCantidadVender"
                  :disabled="productoYaSeleccionado(venta.producto_codigo)"
                >
                  Agregar Producto
                </v-btn>
                <v-btn
          ...
          <div class="form group mt-5" v-if="productosSeleccionados.length > 0">
            <label for="stock mb-4">Productos a Vender:</label>
            <v-row
              ...
              <v-col cols="12">
                <div class="container-producto_seleccionado">
                  <p>
                    ...
                    Precio Unidad: {{ verFormatoMoneda(producto.precio) }}
                    <br />
                    Total Producto:
                    <b>{{
                      verFormatoMoneda(producto.precio * producto.cantidad)
                    }}</b>
                  </p>
                  <v-btn
                    color="danger"
          ...
          <div class="text-end">
            <h5>
              Total: <b>{{ verFormatoMoneda(total) }}</b>
            </h5>
          </div>
    ...
    <div
      class="modal fade"
      id="modalCantidadVender"
      tabindex="-1"
      role="dialog"
      aria-labelledby="modalCantidadVender"
      aria-hidden="true"
      data-backdrop="static"
      data-keyboard="false"
    >
...
export default {
  ...
  methods: {
    ...
    verTotal() {
      const productosAgregados = this.productosSeleccionados;
      let total = 0;
    ...
    agregarProducto() {
      const codigoProductoAgregar = this.venta.producto_codigo;
      if (
        ...
        if (!this.productoYaSeleccionado(codigoProductoAgregar)) {
          ...
          console.log(this.productosSeleccionados, 'productos seleccionados')
          this.cantidadVender = 1;
          this.opcionPrecio === 'PRECIO FINAL'
          this.verTotal();
          $("#modalCantidadVender").modal("hide");
        }
      } else {
    ...
    verProductoSeleccionado(codigo) {
      const cantidad =
        this.cantidadVender === "" ? 1 : parseInt(this.cantidadVender);
      const productos = this.productos;
      const productoSeleccionado = productos.filter(
        (p) => p.codigo === codigo
      )[0];
      const producto = productoSeleccionado
      producto.precio_mostrar = (this.opcionPrecio === 'PRECIO FINAL') ? producto.precio : producto.costo
      let productoSeleccionadoMostrar = {
        codigo: producto.codigo,
        descripcion: producto.descripcion,
        cantidad,
        precio: producto.precio_mostrar,
        total: producto.precio_mostrar * cantidad
      };
      ...
      this.axios
        ...
        .then((respuesta) => {
          if (respuesta.status === 200) {
            this.verProductos();
            this.venta.producto_codigo = respuesta.data.codigo;
    ...
    guardarCliente() {
      this.axios
        ...
        .then((respuesta) => {
          if (respuesta.status === 200) {
            this.verClientes();
            this.venta.cliente_codigo = respuesta.data.codigo;
    ...
    async guardarventa() {
      ...
      const response = await this.axios.post("venta", this.venta);

      const codigoVentaGuardado = response.data.codigo;

      if (response.status === 200 && codigoVentaGuardado > 0) {
        console.log(productosSeleccionados, 'seleccionados')
        if (productosSeleccionados.length > 1) {
          ...
          const promesas = productosSeleccionados.map((producto) => {
            ...
            const registroVenta = {
              ...
              venta_codigo: codigoVentaGuardado,
              desdeprestamo: "N",
              total: producto.total
            };
            // Devolver la promesa resultante de axios.post
            return this.axios
              .post("registro_venta", registroVenta)
              .then((res) => {
                if (res.status === 200) {
                  console.log(res);
                }
          ...
          const registroVenta = {
            cliente_codigo: this.venta.cliente_codigo,
            producto_codigo: productosSeleccionados[0].codigo,
            cantidad: productosSeleccionados[0].cantidad,
            venta_codigo: codigoVentaGuardado,
            desdeprestamo: "N",
            total: productosSeleccionados[0].total
          };
          this.guardarRegistroVenta(registroVenta);
        }
      }
    },
    guardarRegistroVenta(registroVenta) {
      try {
        this.axios
          .post("registro_venta", registroVenta)
          .then((res) => {
    ...
    verProductos() {
      this.venta.usuario_codigo = this.usuario.codigo;
    ...
    validarDatosObligatorios() {
      return (
        ...
        this.venta.cliente_codigo !== undefined
      );
    },
  },
};
